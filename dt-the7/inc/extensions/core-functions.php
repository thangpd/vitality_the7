...
function dt_core_send_mail() {
	...
	if ( ! empty( $fields ) && ! $errors ) {
		...
		// set headers
		$headers = array(
			'From: ' . sanitize_text_field( $name ) . ' <' . $em . '>',
			'Reply-To: ' . $email,
		);
		...
		$subject = apply_filters( 'dt_core_send_mail-subject', sprintf( _x( '[Feedback from: %s]', 'feedback msg', 'the7mk2' ), sanitize_text_field( get_option( 'blogname' ) ) ), $fields );
